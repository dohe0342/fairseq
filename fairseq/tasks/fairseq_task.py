...
class FairseqTask(object):
    ...
    def train_step(
            self, sample, model, criterion, optimizer, update_num, ignore_grad=False, uses_branch_v1=False, uses_branch_v2=False, uses_branch_v3=False, viewmaker=False, multiple_view=False, fgsm=True
    ):
        """
        ...
        if uses_branch_v1:
            ...
            with torch.autograd.profiler.record_function("forward"):
                with torch.cuda.amp.autocast(enabled=(isinstance(optimizer, AMPOptimizer))):
                    loss_list, sample_size, logging_output = criterion(model, sample)
            if ignore_grad:
            ...
            with torch.autograd.profiler.record_function("forward"):
                with torch.cuda.amp.autocast(enabled=(isinstance(optimizer, AMPOptimizer))):
                    if input_fgsm:
                        ...
                        conv_feat_feat, sample_size, logging_output = criterion.forward_and_get_cnn_fgsm(model, sample, optimizer, ignore_grad)
                        loss, sample_size, logging_output = criterion.forward_cnn_fgsm(model, sample, logging_output, cnn_feat)
