...
class FairseqTask(object):
    ...
    def train_step(
        self, sample, model, criterion, optimizer, update_num, ignore_grad=False, uses_branch_v1=False, uses_branch_v2=False, uses_branch_v3=False, multiple_view=False
    ):
        """
        ...
        if uses_branch_v1:
            ...
            if len(optimizer) == 1:
                ...
                with torch.autograd.profiler.record_function("backward"):
                    ...
                    lambda_ = -0.00003*(1+torch.cos(torch.tensor(update_num)/200.))
                    print(update_num, lambda_)
                    optimizer[1].backward(-0.00003*(1+torch.cos(torch.tensor(update_num)/500.))*(loss[0][1])+loss[1]) ## try19
