...
class FairseqTask(object):
    ...
    def train_step(
        self, sample, model, criterion, optimizer, update_num, ignore_grad=False, uses_branch_v1=False, uses_branch_v2=False, uses_branch_v3=False
    ):
        """
            ...
            if len(optimizer) == 1:
                ...
                with torch.autograd.profiler.record_function("backward"):
                    if float(loss[1].data) > 0.0005:
                        ...
                        optimizer[0].backward(loss[0]+loss[1], retain_graph=True)
                        optimizer[1].backward(-0.0001*loss[0]+loss[1])
