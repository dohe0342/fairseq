...
class Viewmaker2(torch.nn.Module):
    ...
    def __init__(self, num_channels=3, distortion_budget=0.05, activation='relu',  
        ...
        self.conv1 = ConvLayer(512+100, 512, kernel_size=2, stride=1)
        self.in1 = torch.nn.InstanceNorm1d(512, affine=True)
        self.conv2 = nn.Conv1d(512, 512, kernel_size=2, stride=1)
    ...
    def basic_net(self, y, num_res_blocks=5, bound_multiplier=1):
        ...
        y = self.add_noise_channel(y, num=100, bound_multiplier=bound_multiplier)
        print(y.size())
        y = self.act(self.in1(self.conv1(y)))
        print(y.size())
        y = self.act(self.in2(self.conv2(y)))
...
class ConvLaye2(torch.nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1):
        super(ConvLayer, self).__init__()
        reflection_padding = kernel_size // 2
        self.reflection_pad = torch.nn.ReflectionPad2d(reflection_padding)
        self.conv1d = torch.nn.Conv2d(
            in_channels, out_channels, kernel_size, stride)
