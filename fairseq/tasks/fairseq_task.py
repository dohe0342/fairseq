...
class FairseqTask(object):
    ...
    def train_step(
            self, sample, model, criterion, optimizer, update_num, ignore_grad=False, uses_branch_v1=False, uses_branch_v2=False, uses_branch_v3=False, viewmaker=True, multiple_view=False, fgsm=False
    ):
        """
        ...
        if uses_branch_v1:
            ...
            with torch.autograd.profiler.record_function("forward"):
                with torch.cuda.amp.autocast(enabled=(isinstance(optimizer, AMPOptimizer))):
                    loss_, sample_size, logging_output = criterion.forward_and_get_fgsm(model, sample, optimizer, ignore_grad)
                    del loss_
