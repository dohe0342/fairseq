...
class MultiheadAttention(nn.Module):
    ...
    def __init__(
        ...
        if fuse:
            ...
            self.k_fused_weight = torch.cat([self.k_proj.weight.T, self.k_proj.bias.unsqueeze(0)]).T

            self.qk_fused_weight = torch.mm(self.k_fused_weight.T, self.q.fused_weight)
            
            self.qk_proj.weight = nn.parameter(self.qk_fused_weight)
    ...
    def forward(
        ...
        if (
            ...
            return F.multi_head_attention_forward(
                ...
                #fuse=self.fuse,
                #fused_w=self.qk_proj if self.qk_proj is None else self.qk_proj.weight
            )
