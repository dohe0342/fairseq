...
class CtcCriterion(FairseqCriterion):
    ...
    def forward(self, model, sample, reduce=True):
        ...
        lprobs_list = [lprobs.contiguous() for lprobs in lprobs_list]  # (T, B, C) from the encoder
        ...
        if not model.training:
            ...
            with torch.no_grad():
                ...
                logging_output[f"wv_errors"] = wv_errs
                logging_output[f"w_errors"] = w_errs
                logging_output[f"w_total"] = w_len
                logging_output[f"c_errors"] = c_err

                logging_output[f"c_total"] = c_len
                ...
                for enum, lprobs in enumerate(lprobs_list):
                    ...
                    logging_output[f"wv_errors_{enum+1}"] = wv_errs
                    logging_output[f"w_errors_{enum+1}"] = w_errs
                    logging_output[f"w_total_{enum+1}"] = w_len
                    logging_output[f"c_errors_{enum+1}"] = c_err
                    logging_output[f"c_total_{enum+1}"] = c_len
    ...
    def reduce_metrics(logging_outputs) -> None:
        ...
        if w_total > 0:
            ...
            metrics.log_derived(
                ...
                if meters["_w_total_1"].sum > 0
                else float("nan"),
            )
            metrics.log_derived(
                "raw_wer1",
                lambda meters: safe_round(
                    meters["_wv_errors_1"].sum * 100.0 / meters["_w_total_1"].sum, 3
                )
                if meters["_w_total_1"].sum > 0
                else float("nan"),
            )
            ...
            metrics.log_derived(
                "wer2",
                lambda meters: safe_round(
                    meters["_w_errors_2"].sum * 100.0 / meters["_w_total_2"].sum, 3
                )
                if meters["_w_total_2"].sum > 0
                else float("nan"),
            )
            metrics.log_derived(
                "raw_wer2",
                lambda meters: safe_round(
                    meters["_wv_errors_2"].sum * 100.0 / meters["_w_total_2"].sum, 3
                )
                if meters["_w_total_2"].sum > 0
                else float("nan"),
            )
            ...
            metrics.log_derived(
                "wer3",
                lambda meters: safe_round(
                    meters["_w_errors_3"].sum * 100.0 / meters["_w_total_3"].sum, 3
                )
                if meters["_w_total_3"].sum > 0
                else float("nan"),
            )
            metrics.log_derived(
                "raw_wer3",
                lambda meters: safe_round(
                    meters["_wv_errors_3"].sum * 100.0 / meters["_w_total_3"].sum, 3
                )
                if meters["_w_total_3"].sum > 0
                else float("nan"),
            )
...
class CtcCriterion(FairseqCriterion):
    ...
    def forward_and_get_cnn_fgsm(self, model, sample, optimizer, ignore_grad=False, update_num=0):
        ...
        if 27211 in sample["id"]:
            if update_num < 362:
                np.save(f'origin.npy', origin_convfeat.detach().cpu().numpy())
...
class CtcCriterionViewmaker(FairseqCriterion):
    def __init__(self, cfg: CtcCriterionConfig, task: FairseqTask):
        super().__init__(task)
        self.blank_idx = (
            task.target_dictionary.index(task.blank_symbol)
    ...
    def reduce_metrics(logging_outputs) -> None:
        ...
        if w_total > 0:
            ...
            metrics.log_derived(
                "wer viewmaker",
                lambda meters: safe_round(
                    meters["_w_errors viewmaker"].sum * 100.0 / meters["_w_total viewmaker"].sum, 3
                )
                if meters["_w_total viewmaker"].sum > 0
                else float("nan"),
            )
            metrics.log_derived(
                "raw_wer viewmaker",
                lambda meters: safe_round(
                    meters["_wv_errors viewmaker"].sum * 100.0 / meters["_w_total viewmaker"].sum, 3
                )
                if meters["_w_total viewmaker"].sum > 0
                else float("nan"),
            )
